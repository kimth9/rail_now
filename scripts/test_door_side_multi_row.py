...
KNOWN_CASES = [
    ...
    # 인천1호선 국제업무지구 — 행선별로 문방향이 갈리는 역(사용자 확인, 2026-08-31).
    # up(송도달빛축제공원 방면)=왼쪽, down(검단호수공원 방면)=오른쪽. 이 역은 인천1호선이
    # arr_sec 자체를 안 남기는 노선이라, 아래 arr_sec 완화 없이는 애초에 반영 자체가 안 됨.
    ("IC1#토휴일_하선#1001", "국제업무지구", "왼쪽"),
    ("IC1#토휴일_상선#1008", "국제업무지구", "오른쪽"),
    # 인천1호선 부평(단일행 역) — arr_sec 완화 전에는 이 노선 전체가 중간역 door_side가
    # 하나도 안 나왔음(dep_sec는 있지만 arr_sec가 없어서 필터에 걸림).
    ("IC1#토휴일_상선#1004", "부평", "오른쪽"),
]
