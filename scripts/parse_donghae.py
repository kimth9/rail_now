...
입력 구조 (관측):
  ...
  - 역명 절삭은 대부분 단순 3글자 프리픽스(`오시리`→오시리아, `신해운`→신해운대,
    `부산원`→부산원동, `거제해`→거제해맞이). 예외로 `부교대`→`교대`는 부산 도시철도
    3호선에도 같은 이름의 역이 있어 붙는 구분 접두사로 추정(나무위키 역별 승하차
    순위표에 실제 표기가 '교대'로 확정됨). 전부 나무위키 대조 완료, 미확인 없음.
"""
import csv
import datetime
import os
...
STATION_ALIAS = {
    ...
    "부산원": ("부산원동", "✔"),
    "거제해": ("거제해맞이", "✔"),
    "부교대": ("교대", "✔"),   # 부산 도시철도 3호선 교대역과 구분하는 접두사로 추정
}
