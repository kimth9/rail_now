#!/usr/bin/env python3
"""서울지하철 5호선(서울교통공사) 시각표 xlsx -> 정규화 테이블

평일/휴일이 완전히 별개 파일(시트명도 다름: 평일 파일은 "5호선 평일 상행/하행",
휴일 파일은 "휴_상행/휴_하행") — daytype을 파일명이 아니라 인자로 받는다.

구조: 1행="열차번호"(헤더 마커), 2행="출발역", 3행="도착역"(둘 다 정보용, 실제
파싱엔 불요 — origin/destination은 데이터에서 직접 뽑음), 4행부터 역.
A열=역명(새 역 시작 행에만) / B열="도착"/"출발" / C열부터 열차별 시각(텍스트).
"""
import csv
import os
import re
...
SRC = sys.argv[1] if len(sys.argv) > 1 else "RAW/241202_붙임. 5호선 열차운행시각표(평일_241202).xlsx"
OUT = sys.argv[2] if len(sys.argv) > 2 else "tmp/out_seoul5"
DAYTYPE = sys.argv[3] if len(sys.argv) > 3 else "평일"

TIME_RE = re.compile(r"^(\d{1,2}):(\d{2}):(\d{2})$")
NOT_A_STATION = {"연결열차", "연결열번"}
# 휴일 원본(2023-10-07)에만 남은 오타·구 역명 — 평일 원본과 대조해 확정
RENAME = {
    "개농": "개롱",              # 오타
    "동대문운동장": "동대문역사문화공원",  # 2009년 개명 전 구 역명
}
        ...
        for rr in range(first_station_row, ws.max_row + 1):
            ...
            if label:
                ...
                if current not in stops:
                    stops[current] = f"S5M{len(stops) + 1:04d}"
